...
from statline.core.adapters.load import load_spec as _load_spec
from statline.core.adapters.sniff import sniff_adapters as _sniff_adapters
from statline.core.datasets import list_datasets as _list_datasets

# IMPORTANT: calc endpoints below expect MAPPED metrics
from statline.core.scoring import calculate_pri as _calculate_pri
from statline.core.scoring.map import safe_map_raw as _safe_map_raw
from statline.gateway.adapters.discovery import list_discoverable_yaml as _list_yaml
from statline.gateway.adapters.score_types import ScoreBatchRequest, ScoreRowRequest
from statline.gateway.adapters.scoring import adapters_available as _adapters_available
from statline.gateway.adapters.scoring import score_batch as _score_batch
from statline.gateway.adapters.scoring import score_row as _score_row
...
# =============================================================================
# Minimal auth schemas (until you move them into statline.gateway.http.models)
# =============================================================================
...
def list_adapters(fast: bool = False) -> Dict[str, List[str]]:
    if fast:
        return {"adapters": _list_yaml() or list(_list_adapters())}

    try:
        names = _adapters_available() or list(_list_adapters())
        if names:
