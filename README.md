...
\`\`\`python
from statline import list\_adapters, load\_dataset, score
print(list\_adapters())
rows = load\_dataset("DEMO/demo", *limit*=10)
results = score("demo", rows, *mode*="batch", *weights*="pri")
for row in results[:3]:
    print(row["pri"], row["pri\_raw"], row\.get("scores", {}))
\`\`\`
For one row:
\`\`\`python
from statline import score\_row
player = {
    "name": "Example Player",
    "ppg": 24.5,
    "apg": 6.2,
...
result = score\_row("demo", player, *weights*="pri")
print(result["pri"])
\`\`\`
\---
\## CLI overview
...
\| \`--url URL\` | Set the SLAPI base URL. Also supported through \`SLAPI\_URL\`. |
...
\### PRI and \`pri\_raw\`
\`pri\_raw\` is the normalized raw score. \`pri\` is the adapter/profile-rendered score. Some adapters also expose additional profile scores such as \`pri\_af\`, \`pri\_ar\`, or \`pri\_ap\`.
\---
\## Input formats
...
\`\`\`bash
SLAPI\_HOST=127.0.0.1 SLAPI\_PORT=8000 slapi
\`\`\`
Then point clients at it:
\`\`\`bash
*export* SLAPI\_URL="[http://127.0.0.1:8000](http://127.0.0.1:8000)"
...
statline auth enroll --token reg\_... --user your-handle --email [you@example.com](mailto\:you@example.com)
statline auth apikey-request --owner your-name
statline auth apikey-claim --request-id REQUEST\_ID
...
\---
\## Repository
\- Homepage: \`[https://statline.dev](https://statline.dev)\`
