...
'@ | Set-Content -LiteralPath (Join-Path $binRoot "statline.cmd") -Encoding Ascii

    @'
@echo off
"%~dp0..\runtime\python.exe" -c "from statline.app.runner.main import main; main()" %*
'@ | Set-Content -LiteralPath (Join-Path $binRoot "slapi.cmd") -Encoding Ascii
...
Root: HKCU; Subkey: "Software\Classes\StatLine.StatPack\shell\open\command"; ValueType: string; ValueName: ""; ValueData: """{app}\runtime\python.exe"" -m statline run --pause ""%1"""
